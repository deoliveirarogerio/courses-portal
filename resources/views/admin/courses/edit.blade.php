...
@section('title', 'Editar Curso - Painel Administrativo')
    ...
    <div class="col-12">
        <div class="d-flex justify-content-between align-items-center">
            <div>
                ...
                <p class="text-muted mb-0">Atualize as informações do curso</p>
            </div>
            <div>
        ...
        <form action="{{ route('admin.courses.update', $course) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-8">
                    ...
                    <div class="mb-3">
                        ...
                        <textarea class="form-control @error('curriculum') is-invalid @enderror" 
                                  id="curriculum" name="curriculum" rows="6">{{ old('curriculum', $course->curriculum) }}</textarea>
                        @error('curriculum')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        @if($course->image)
                            <div class="mb-2">
                                <img src="{{ $course->image_url }}" alt="Imagem atual" 
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('difficulty_level') is-invalid @enderror" 
                            ...
                            <option value="iniciante" {{ old('difficulty_level', $course->difficulty_level) == 'iniciante' ? 'selected' : '' }}>Iniciante</option>
                            <option value="intermediario" {{ old('difficulty_level', $course->difficulty_level) == 'intermediario' ? 'selected' : '' }}>Intermediário</option>
                            <option value="avancado" {{ old('difficulty_level', $course->difficulty_level) == 'avancado' ? 'selected' : '' }}>Avançado</option>
                        </select>
                    ...
                    <div class="mb-3">
                       ...
                       <select class="form-select @error('instructor') is-invalid @enderror" id="instructor_id" name="instructor_id" required> 
                            @foreach($instructors as $instructor)
                                <option value="{{ $instructor->id }}" {{ old('instructor_id', $instructor->id) == $course->instructor_id ? 'selected' : '' }}>
                                    {{ $instructor->name }}
                                </option>
                            @endforeach   
                        </select>   
                    ...
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('status') is-invalid @enderror" 
                            ...
                            <option value="">Selecione...</option>
                            <option value="ativo" {{ old('status', $course->status) == 'ativo' ? 'selected' : '' }}>Ativo</option>
                            <option value="inativo" {{ old('status', $course->status) == 'inativo' ? 'selected' : '' }}>Inativo</option>
                            <option value="rascunho" {{ old('status', $course->status) == 'rascunho' ? 'selected' : '' }}>Rascunho</option>
                        </select>
                        @error('status')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
            ...
            <div class="d-flex justify-content-end gap-2">
                ...
                <button type="submit" class="btn btn-primary">
                    <i class="bi bi-check-circle me-2"></i>Atualizar Curso
                </button>
            </div>
        </form>
    </div>
